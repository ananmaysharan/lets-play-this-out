...
export function Ballot({
  ...
  function handleKey(e: KeyboardEvent<HTMLDivElement>, id: string) {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      (document.querySelector(`[data-vote="${next.id}"]`) as HTMLElement | null)?.focus();
    } else if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
      e.preventDefault();
      const idx = options.findIndex((o) => o.id === id);
      const prev = options[(idx - 1 + options.length) % options.length];
      (document.querySelector(`[data-vote="${prev.id}"]`) as HTMLElement | null)?.focus();
    }
  }
  ...
  return (
    <>
      <div className="ballot-frame">
        ...
        <div className="ballot-question">{question}</div>
        <div className="ballot-instruction">Mark your choice with an X</div>
            ...
            return (
              ...
              >
                <div className="ballot-checkbox">
                  {isSel ? (
                    <svg key={selected} className="ballot-mark" viewBox="0 0 40 40">
        ...
        <div className="ballot-footer">Precinct 2760 · Polling Station NYC</div>
      </div>
